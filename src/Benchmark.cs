...
namespace Portfish
{
    ...
    internal static class Benchmark
    {
        ...
        internal static void benchmark(Position current, Stack<string> stack)
        {
            ...
            var res = new long[fens.Count];
            for (var i = 0; i < fens.Count; i++)
            {
                time.Reset();
                time.Start();
                var pos = new Position(fens[i], bool.Parse(OptionMap.Instance["UCI_Chess960"].v), Threads.main_thread());
                ...
                if (limitType == "perft")
                {
                    var cnt = Search.perft(pos, limits.depth * DepthC.ONE_PLY);
                    Plug.Write("\nPerft ");
                ...
                {
                    Threads.start_thinking(pos, limits, new List<int>());
